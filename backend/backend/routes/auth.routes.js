...
authRoute.get(
    ...
    //  const token = jwt.sign({ user }, process.env.SECRET, { expiresIn: '1hr' })
        const token = jwt.sign({ user }, process.env.JWT_SECRET, { expiresIn: '1hr' })
          ...
          res.redirect(`http://localhost:3000/dashboard?google=true&id=${user._id}&token=${token}&profilePic=${user.profilePic}`);
...
authRoute.get("/google/failure", (req, res) => {
    res.redirect("http://localhost:3000/unauthenticated")
    ...
    new GoogleStrategy(
        {
            ...
            callbackURL: "http://localhost:5000/auth/google/callback", // change the callback link
